"""TDQS (Tool Definition Quality Score) Evaluator for Intervals.icu MCP Server.

Specification & Standard: https://github.com/glama-ai/tool-definition-quality-score
"""

from typing import Any, Dict
from intervals_mcp.server import mcp
...
def evaluate_tdqs() -> None:
    ...
    for name, tool in tools.items():
        ...
        has_usage = (
            ...
            or "fetch" in doc.lower()
            or "execute" in doc.lower()
            or "search" in doc.lower()
            or "update" in doc.lower()
            or "add" in doc.lower()
            or "get" in doc.lower()
            or "upload" in doc.lower()
            or "download" in doc.lower()
        )
        if has_usage:
